...
We are working with Professor Barresi's lab in trying to create a plugin for Imaris to automate data analyses on 3D and 4D images. IMARIS is an Interactive Microscopy Image Analysis software offered by Bitplane that allows users to process and analyze fluorescent 3D and 4D microscopy images [@GAUTIER2021109038]. Currently, analysis of data requires users to manually create reference points and surfaces across the image to gather information. However, such a process can be lengthy and difficult, especially for people who are new to IMARIS, and can be more difficult to replicate in research studies. As such, a plugin that automates the data processing is needed.
